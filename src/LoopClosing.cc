...
LoopClosing::LoopClosing(Atlas *pAtlas, KeyFrameDatabase *pDB, ORBVocabulary *pVoc, const bool bFixScale, const bool bActiveLC):
    ...
    nMerges(0), nLoop(0), nFGBA_exec(0), nFGBA_abort(0),
#endif
    mbResetRequested(false), mbResetActiveMapRequested(false), mbFinishRequested(false), mbFinished(true), mpAtlas(pAtlas),
    mpKeyFrameDB(pDB), mpORBVocabulary(pVoc), mnCovisibilityConsistencyTh(3), mpLastCurrentKF(static_cast<KeyFrame*>(NULL)),
    mpMatchedKF(NULL), mbLoopDetected(false),
    mnLoopNumCoincidences(0), mnLoopNumNotFound(0), mbMergeDetected(false), mnMergeNumCoincidences(0), mnMergeNumNotFound(0), 
    ...
    mstrFolderSubTraj("SubTrajectories/"), mnNumCorrection(0), mnCorrectionGBA(0), mbActiveLC(bActiveLC)
{
}
